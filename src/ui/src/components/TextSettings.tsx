...
const TextSettings: React.FC<TextSettingsProps> = ({ open, settings, onSubmit, onClose }) => {
    const [temperature, setTemperature] = React.useState<number>(settings.temperature);
    const [topP, setTopP] = React.useState<number>(settings.top_p);
    const [stopSequences, setStopSequences] = React.useState<string>(settings?.stop?.join("\n"));
    ...
    const handleSubmit = () => {
        ...
        if(submitSettings.stop != null && submitSettings.stop?.length > 0) {
            submitSettings.stop = stopSequences.split("\n").map(s => s.trim()).filter(s => s.length > 0);
        }
        ...
        >
            <Stack sx={{ width: '100%', padding: "1rem" }} spacing={2}>
                <Typography variant="h5">
                    Settings for Text Completions
                </Typography>
                <Divider />
                <FormControl variant="standard" sx={{ m: 1, mt: 3 }}>
                    <FormHelperText id="text-temp-helper-text">
                ...
                <FormControl variant="standard" sx={{ m: 1, mt: 3 }}>
                    <FormHelperText id="text-topp-helper-text">
                        <Typography variant="body1" style={{color: colors.blue[500]}}>
                            Top P:  {topP}
                        </Typography>
                        <Typography variant="body2">
                ...
                <FormControl variant="standard" sx={{ m: 1, mt: 3 }}>
                    <FormHelperText id="text-max-tokens-helper-text">
                        <Typography variant="body1" style={{color: colors.blue[500]}}>
                ...
                <FormControl variant="standard" sx={{ m: 1, mt: 3 }}>
                    ...
                    <Typography variant="body2">
                        How many completions to generate for each prompt. Because this parameter generates many completions, it can quickly consume your token quota. Use carefully and ensure that you have reasonable settings for max_tokens and stop.
                    </Typography>   
                </FormControl>
                <FormControl variant="standard" sx={{ m: 1, mt: 3 }}>
                    <Typography variant="body1" style={{color: colors.blue[500]}}>
                ...
                <FormControl variant="standard" sx={{ m: 1, mt: 3 }}>
                    <FormHelperText id="text-stop-helper-text">
                        <Typography variant="body1" style={{color: colors.blue[500]}}>
                            Stop Sequence:
                        </Typography>
                        <Typography variant="body2">
